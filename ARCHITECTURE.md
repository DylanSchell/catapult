...
```
catapult/
├── src-tauri/src/           # Rust backend (3,900+ LOC)
...
│   ├── huggingface.rs       # HF API search, recommended models list, quant extraction
│   └── main.rs              # Entry point stub
├── src/                     # React/TypeScript frontend (4,100+ LOC)
│   ├── App.tsx              # Router (wizard + main layout)
│   ├── main.tsx             # React entry
│   ├── pages/
│   │   ├── Dashboard.tsx    # System overview, quick launch, model management
...
└── tests
    ├── (Rust)               # 31 unit tests in #[cfg(test)] modules
    └── src/utils/format.test.ts  # 31 Vitest unit tests
```
...
`start_server` spawns `llama-server` with `kill_on_drop(true)`. The child process is stored in `ServerState` (behind a Mutex). Stdout/stderr are read by independent tokio tasks that emit `server_log` events and buffer up to 500 lines. The full command line is stored as the first log entry.
...
Status transitions: `Stopped → Starting → Running` (detected by "HTTP server listening" in output) or `Starting → Error` on process exit.
...
- **Rust (31 tests):** `cargo test` — unit tests in `#[cfg(test)]` modules covering asset scoring, backend detection, CLI arg building, quant extraction, size estimation, filename parsing, GGUF parsing, and hardware config suggestions
- **TypeScript (31 tests):** `npm test` (Vitest) — utility function tests for CPU/GPU name shortening, size formatting, quant color/sort mapping, and imatrix detection
